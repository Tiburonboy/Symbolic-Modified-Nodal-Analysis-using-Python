...
Google Colab can be run on a mobile device such as a mobile phone. @sec-SMNA_mobile is an example of a Jupyter notebook that can be uploaded to Colab and run to perform symbolic circuit analsys on a mobile device. Of course internet connectivity is required. The circuit shown in @fig-EasyEDA-example is used in @sec-SMNA_mobile.
