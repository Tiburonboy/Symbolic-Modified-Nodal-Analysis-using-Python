...
This is the website is my online book - *Symbolic Modified Nodal Analysis using Python*. Symbolic circuit analysis is a circuit analysis method that derives network equations with the circuit elements represented by symbols and the topology of the circuit described by a netlist.

The circuit analysis technique presented in this book covers circuits driven with sinusoidal sources where the DC or AC steady state response is obtained. The inductors and capacitors in the circuit are replace by their transformed values. Inductors are replace by their steady state Laplace value, $Ls$, where L is the value of the inductor with units of henrys (named after [Joseph Henery](https://en.wikipedia.org/wiki/Joseph_Henry)) and $s$ is the Laplace variable equal to $j \omega$ where $\omega$ is the radian frequency. Capacitors in the circuit are replace by their steady state Laplace value, $1/(Cs)$, where C is the value of the capacitor with units in farads (named after [Micheal Faraday](https://en.wikipedia.org/wiki/Michael_Faraday)). 
...
- [ ] use N for network equations (N_sym, N), U for solved system of equations and H for system functions
